...
from pcfg_dataset import TextDataset, pcfg_collate_fn, save_idx2token
...
def parse_args_and_config():
    ...
    parser.add_argument("--txt_file_name", type=str, default=None,
                        help="Path to text file of real sentences")
...
def main():
    ...
    #token2idx, idx2token = create_token_mappings(pcfg, special_tokens)
    ...
    #train_dataset = PCFGDataset(pcfg, num_samples=args.train_size, max_length=50, 
                            #  token2idx=token2idx, special_tokens=special_tokens)
    ...
    # Train the model
    train_model(model, train_loader, val_loader, train_config, args.model_save_dir)       
 
    # Test the model
    test_loss = evaluate_model(model, test_loader, device)
    print(f"Test loss: {test_loss:.4f}")

    # Add test loss to losses.json
