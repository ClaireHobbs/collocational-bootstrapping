...
def plot_single_experiment_results(experiment_dir, output_path=None):
    """Plot results from a single parameter experiment as a bar chart"""
    ...
    # Auto-detect distribution type from directory name
    dir_name = experiment_path.name
    distribution_type, param_value = parse_experiment_directory(dir_name)
    ...
    with open(summary_file, 'r') as f:
        for line in f:
            ...
            if ('Experiment Summary for' in line and 'distribution' in line) or ('C=' in line and 'Experiment Summary' in line):
                # New format: "Experiment Summary for geometric distribution: C=0.5"
                # Old format: "Experiment Summary for C=0.5"
                if 'distribution:' in line:
                    param_info = line.split('distribution: ')[1]
                elif 'C=' in line:
                    param_info = line.split('C=')[1].split()[0]
                    param_info = f"C={param_info}"
    ...
    # Create appropriate title based on detected or parsed parameter info
    if param_info:
        title = f'Model Accuracy by Evaluation Type ({param_info})'
    elif distribution_type and param_value is not None:
        param_name = get_parameter_name(distribution_type)
        title = f'Model Accuracy by Evaluation Type ({param_name} = {param_value})'
    else:
        title = 'Model Accuracy by Evaluation Type'
    ...
    summary_title = param_info if param_info else f"{get_parameter_name(distribution_type)} = {param_value}" if (distribution_type and param_value is not None) else "Experiment Results"
    print(f"\nResults Summary for {summary_title}:")
    print("=" * 40)
